      ...
      <div style={{background:"#0a1628",border:"1px solid rgba(251,191,36,0.3)",borderRadius:20,padding:32,width:"100%",maxWidth:560,boxShadow:"0 20px 60px rgba(0,0,0,0.6)"}}>
        ...
        <div style={{background:productionLocked?"rgba(59,130,246,0.12)":"rgba(16,185,129,0.08)",border:`1px solid ${productionLocked?"rgba(59,130,246,0.28)":"rgba(16,185,129,0.2)"}`,borderRadius:12,padding:14,marginBottom:18}}>
          <div style={{fontSize:12,color:productionLocked?"#93c5fd":"#10b981",fontWeight:700,marginBottom:6}}>
            {productionLocked ? "🌐 Production site detected" : "📋 Follow the setup guide — then paste your URL below"}
          </div>
          <div style={{fontSize:11,color:"rgba(255,255,255,0.5)",lineHeight:1.7}}>
            {productionLocked ? (
              <>
                Recommended production backend:<br/>
        ...
        {usingOverride && (
          <div style={{background:"rgba(245,158,11,0.10)",border:"1px solid rgba(245,158,11,0.24)",borderRadius:12,padding:14,marginBottom:18}}>
            ...
            <div style={{fontSize:11,color:"rgba(255,255,255,0.58)",lineHeight:1.6}}>
              This browser is using a custom Apps Script URL instead of the recommended backend.
            </div>
          </div>
        )}
...
export default function App() {
  ...
  const SCRIPT_OVERRIDE_KEY = "nandanam_script_url_override_v1";
  const readScriptUrl = ()=>{
    try{
      const stored = localStorage.getItem("nandanam_script_url") || "";
      const hasExplicitOverride = localStorage.getItem(SCRIPT_OVERRIDE_KEY) === "1";
      if(isProductionHost && !hasExplicitOverride){
  ...
  return (
    <div style={{fontFamily:"'DM Sans',sans-serif",minHeight:"100vh",background:"#060d1a",color:"#fff"}}>
      ...
      {showSheets&&<ScriptModal current={scriptUrl} defaultUrl={DEFAULT_SCRIPT_URL} productionLocked={isProductionHost} onSave={url=>{
        setScriptUrl(url);
        try{
          localStorage.setItem("nandanam_script_url",url);
          if(url && url !== DEFAULT_SCRIPT_URL) localStorage.setItem(SCRIPT_OVERRIDE_KEY,"1");
          else localStorage.removeItem(SCRIPT_OVERRIDE_KEY);
        }catch{}
        setShowSheets(false);
